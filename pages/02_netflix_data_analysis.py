...
n_countries = movies_df["country"].dropna().nunique()
...
movies_only = movies_df[movies_df["type"] == "Movie"]  # filtrer films
movies_only["duration_minutes"] = movies_only["duration"].str.replace(" min", "").astype(int)

movies_avg_duration_per_year = movies_only.groupby("release_year")["duration_minutes"].mean()  # TODO


if movies_avg_duration_per_year is not None:
    fig = plt.figure(figsize=(9, 6))

    plt.plot(movies_avg_duration_per_year.index, movies_avg_duration_per_year.values)  # TODO
